...
import com.eplayer.R;

import java.util.ArrayList;
import java.util.List;

/**
 * Created by lxr on 2018/3/20.
 */
...
public class FlowLayout extends ViewGroup {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        // 计算最大的宽度，指的是除去左右两边的边距，能被child填充的最大宽度
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        l = getPaddingLeft();
        t = getPaddingTop(); //指的是父布局为子view设定的padding值，是相对于父布局的padding值
        for (int i = 0; i < mLines.size(); i++) {
            // 获取行
    ...
    public class Line {
        ...
        public void addView(View view) {
            ...
            if (views.size() == 0) {
                // 集合里没有孩子的时候
                if (childWidth > maxWidth) { //如果添加孩子的宽度大于行的最大宽度
                    usedWidth = maxWidth;
                    height = childHeight;
                } else {
                ...
                height = childHeight > height ? childHeight : height; //取大的那个值
            }
