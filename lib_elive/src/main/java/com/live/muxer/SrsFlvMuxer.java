...
public class SrsFlvMuxer {
    ...
    private class SrsRawH264Stream {
        ...
        /**
         * H264 NALU type Slice 封装成 FLV video tag
         * 第一：封装 video tag data header，需要5个byte，
         * 1）FrameType | CodecID     //11                           0x27 or 0x17
         * 2）AVCPacketType           //12                           0x01
         * 3）Composition Time        //13-15                        0x00 0x00 0x00
         *  其中数字11表示在整个flv tag的字节序列中位置是11，前面0-10个字节属于FLV tag header数据，flv tag header的数据在rtmp publisher模块中封装
         *
         *  第二：封装nalu数据，依次包括4byte的nalu header和nalu data
         *
         * @param frames
         * @param frame_type
         * @param avc_packet_type
         * @param dts
         * @param pts
         * @return
         */
        ...
        private SrsAnnexbSearch searchStartcode(ByteBuffer bb, MediaCodec.BufferInfo bi) {
            ...
            if (bi.size - 4 > 0) {
                ...
                }else if (bb.get(0) == 0x00 && bb.get(1) == 0x00 && bb.get(2) == 0x01){ //3字节
        ...
        public SrsFlvFrameBytes demuxAnnexb(ByteBuffer bb, MediaCodec.BufferInfo bi, boolean isOnlyChkHeader) {
            ...
            if (bb.position() < bi.size - 4) {
                ...
                if (!tbbsc.match || tbbsc.nb_start_code < 3) {
                    ...
                    // the start codes. 去掉起始码之前的数据（包括起始码）
                    for (int i = 0; i < tbbsc.nb_start_code; i++) {
                        bb.get(); //取出当前pos的byte，然后pos增加1
                    }
    ...
    private class SrsFlv {
        ...
        public void setAudioTrack(MediaFormat format) {
            audioTrack = format;
            achannel = format.getInteger(MediaFormat.KEY_CHANNEL_COUNT); //声道
            asample_rate = format.getInteger(MediaFormat.KEY_SAMPLE_RATE); //采样率
        }
        ...
        public void writeAudioSample(final ByteBuffer bb, MediaCodec.BufferInfo bi) {
            ...
            int dts = pts;
            //分配一个比音频数据大2个byte的内存空间
            audio_tag = mAudioAllocator.allocate(bi.size + 2);
            byte aac_packet_type = 1; // 1 = AAC raw ，aac原始数据
            //如果没发送配置信息则发送
            if (!aac_specific_config_got) {
                ...
                byte samplingFrequencyIndex = 0x04; //默认44100，二进制为0000 0100
                ...
                //0x07=0000 0111，当A按位与0x07的时候，相当于取A的后3位值，ch按位或再赋值符号|= 0000 0100的时候，如果ch后面3位是000的话，相当于
                //将100加到ch的后3位上
                ...
                // channelConfiguration; 4 bslbf，声道信息占4个bit
                byte channelConfiguration = 1; //0000 0001
                if (achannel == 2) {
                    //0000 0010
                    channelConfiguration = 2;
                }
                //0x78=0111 1000,0000 0010左移3位以后变为0 0010 000
                ...
                //写音频原始数据，从位置2开始写将bb的数据复制到audio_tag中，其中前面2位byte是留给audio_header和aac_packet_type的
            ...
            byte audio_header = (byte) (sound_type & 0x01); //右1的bit
            audio_header |= (sound_size << 1) & 0x02; //右2的bit
            audio_header |= (sound_rate << 2) & 0x0c; //0x0c=1100，2个bit
            audio_header |= (sound_format << 4) & 0xf0; // 0xf0=1111 0000，取高4位bit
        ...
        public void writeVideoSample(final ByteBuffer bb, MediaCodec.BufferInfo bi) {
            ...
            //解封装annexb结构，这里主要是去掉起始码并移动bb的pos
            SrsFlvFrameBytes frame = avc.demuxAnnexb(bb, bi, true);
            //nal类型
            int nal_unit_type = frame.data.get(0) & 0x1f;

            if (nal_unit_type == SrsAvcNaluType.IDR) { //关键帧
                type = SrsCodecVideoAVCFrame.KeyFrame;
            } else if (nal_unit_type == SrsAvcNaluType.SPS || nal_unit_type == SrsAvcNaluType.PPS) {
                //Log.d(TAG, "writeVideoSample: 发送sps");
                // 一般ByteBuffer数据的顺序为：sps+pps+sei
                // start code 一般是3字节(0x000001)或者4字节（0x00000001），在sps和pps和Access Unit的第一个NALU使用4字节起始码，其余情况均使用3字节起始码
                //这里主要是获取pps的frame数据，即sps后面的数据，同时bb的pos也被移动
                SrsFlvFrameBytes frame_pps = avc.demuxAnnexb(bb, bi, false);
                //此时frame.size为sps的大小
                frame.size = frame.size - frame_pps.size - 4;  // 4 ---> 00 00 00 01 pps
                if (!frame.data.equals(h264_sps)) { //说明frame的数据不是pps
...
//                    writeH264SpsPps(dts, pts);
        ...
        private void writeRtmpPacket(int type, int dts, int frame_type, int avc_aac_type, SrsAllocator.Allocation tag) {
            ...
            if (frame.isVideo()) {
                //Log.d(TAG, "writeRtmpPacket: 帧类型="+frame_type);
                //刚开始需要找到关键帧，即从关键帧开始保存
