...
void VideoEncoder::putAvData(AvData *data) {
    LOGD("压缩后YUV数据开始保存");
    MediaEncoder::putAvData(data);
    LOGD("压缩后YUV数据保存成功");
}
...
int VideoEncoder::encodeFrame(uint8_t *inBytes, int frameSize, int pts, uint8_t *outBytes, int *outFrameSize) {
    ...
    // 分别定位 Y、U、V 的指针
    uint8_t *i420_y_data = inBytes;
    uint8_t *i420_u_data = inBytes + i420_y_size;
    uint8_t *i420_v_data = inBytes + i420_y_size + i420_u_size;
    ...
    memcpy(pic_in.img.plane[2], i420_v_data, i420_v_size); // 保存V数据
