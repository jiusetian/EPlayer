...
void RtmpLivePush::pushVideoData(AvData* data) {
    ...
    for (int i = 0; i < nulNum; ++i) {
        ...
        // start code
        int offset = 4;
        if (nalBytes[0] == 0x00 && nalBytes[1] == 0x00 && nalBytes[2] == 0x01) {
            offset = 3;
        }
        // nal type
        int type = nalBytes[offset] & 0x1f;

        if (type == NAL_SPS) {
            spsLen = nalLen - offset;
            sps = new uint8_t[spsLen];
            memcpy(sps, nalBytes + offset, spsLen);
        } else if (type == NAL_PPS) {
            ppsLen = nalLen - offset;
            pps = new uint8_t[ppsLen];
            memcpy(pps, nalBytes + ppsLen, ppsLen);
            // send sps and pps
...
void RtmpLivePush::pushAudioData(AvData* data) {
    ...
    if (!isSendAudioHeader) {
        isSendAudioHeader = true;
        addSequenceAacHeader(44100, 2, 0);
    }
    // send the body
    addAccBody(audioData, audioLen, 0);
